...
.controller('HomeGKSave_Ctrl', function($scope, $state, $http, $stateParams, Matches, HomeTeams, Teams, Players, PlayerSeasons) {
  ...
  PlayerSeasons.query().$promise.then(function(response){
    ...
    $scope.submit_home = function(id) {
      ...
      var player_season_that_scored_gk_saves = getPlayerSeasonById($scope.player_seasons, id);
      var gk_save = 'http://192.168.1.104:3000/api/v1/goalkeeper_saves';
      
      $http.post(gk_save,{ player_season_id : player_season_that_scored_gk_saves,
