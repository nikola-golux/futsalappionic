...
/****************************************************************************************************
* HOME YELLOW CARDS CONTROLLERS
****************************************************************************************************/
.controller('HomeYellowCard_Ctrl', function($scope, $state, $http, $stateParams, Matches, HomeTeams, Teams, Players, PlayerSeasons, MatchPlayers) {
  ...
  PlayerSeasons.query().$promise.then(function(response){
    ...
    $scope.submit_home = function(id) {
      ...
      var player_season_that_scored_yellow_cards = getPlayerSeasonById($scope.player_seasons, id);
      var yellow_card = 'http://192.168.1.104:3000/api/v1/yellow_cards';
      
      $http.post(yellow_card,{ player_season_id : player_season_that_scored_yellow_cards,
...
/****************************************************************************************************
* AWAY YELLOW CARDS CONTROLLERS
****************************************************************************************************/
.controller('AwayYellowCard_Ctrl', function($scope, $state, $http, $stateParams, Matches, AwayTeams, Teams, Players, PlayerSeasons, MatchPlayers) {
  Matches.query().$promise.then(function(response){
    $scope.id_meca = localStorage.getItem('current_match');
    $scope.matches = response;
    /* Funkcija za nalazenje JSON elementa */
